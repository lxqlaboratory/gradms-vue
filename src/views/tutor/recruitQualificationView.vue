...
import { recruitQualificationView } from '@/api/tutor'
...
export default {
  ...
  methods: {
    fetchData() {
      ...
      recruitQualificationView({ 'session': document.cookie }).then(res => {
    ...
    getApplyList() {
      recruitQualificationView({ 'session': document.cookie, 'year':this.year, 'collegeId':this.collegeId, 'perNum':this.perNum, 'perName':this.perName,'state':this.state }).then(res => {
        this.applyList = res.data.applyList
        this.applyIds = res.data.applyIds
      })
    },
